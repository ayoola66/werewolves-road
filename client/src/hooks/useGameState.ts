      ...
      if (now >= endTime) {
        try {
          if (currentPhase === 'night') {
            ...
            // Transition from role_reveal to night
            // This will be handled by process-night or we need a separate function
            // For now, just refresh state - the server should handle this transition
            await fetchGameState(gameState.game.gameCode);
          }
        } catch (error: any) {
